...
func parseCommandsIntoSignals(cmd map[string]string, target string, signals map[string]uint16) map[string]uint16 {
	fmt.Println("Entering: ", target, cmd[target])
	...
	if len(strcmd) == 1 {
		...
		if err != nil {
			fmt.Println("In base case received error:", err)
			return nil
		}
		signals[target] = uint16(n)
		return signals
	}
	// RECURSIVE CASES
	...
	if strings.Contains(strcmd[1], "AND") || strings.Contains(strcmd[1], "OR") {
	...
	if len(strcmd) == 3 && strings.Contains(strcmd[1], "SHIFT") {
		...
		if err != nil {
			fmt.Println("Unable to parse into string", err)
			return nil
		}
		num = n
	}
	...
	if strings.Contains(strcmd[1], "AND") {
		signals[target] = signals[strcmd[0]] & signals[strcmd[2]]
		return signals
	}
	if strings.Contains(strcmd[1], "OR") {
		signals[target] = signals[strcmd[0]] | signals[strcmd[2]]
	}
	if strings.Contains(strcmd[0], "NOT") {
